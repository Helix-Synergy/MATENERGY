...
const Navbar = () => {
  ...
  const isActive = (link) => {
    ...
    return location.pathname.replace(/\/+$/, "") === link.replace(/\/+$/, "");
  };
      ...
      <div className="flex items-center justify-between md:px-6 px-4 py-2 md:py-2 transition-all duration-300">
        ...
        >
          <img
            ...
            style={{
              height: isOpen && window.innerWidth < 1024 ? "5rem" : scrolled ? "5rem" : "8rem",
              width: isOpen && window.innerWidth < 1024 ? "5rem" : scrolled ? "6rem" : "8rem",
              marginTop: isOpen && window.innerWidth < 1024 ? "0.5rem" : scrolled ? "0rem" : "1rem",
              border: isOpen && window.innerWidth < 1024 ? "none" : scrolled ? "none" : "2px solid #3FA256",
              backgroundColor: isOpen && window.innerWidth < 1024 ? "transparent" : scrolled ? "transparent" : "white",
              padding: isOpen && window.innerWidth < 1024 ? "0px" : scrolled ? "0px" : "4px",
            }}
          />
        </div>
        ...
        >
          {nav_links.map((item, index) => (
            <Link
              key={index}
              to={item.link}
              className={`hover-underline-animation transition pb-1 ${isActive(item.link)
                ? "text-one font-bold border-b-2 border-one" // Active state
                : "text-gray-700 hover:text-one border-b-2 border-transparent" // Default state
                }`}
              style={{
        ...
        <div className="lg:hidden bg-white px-4 pb-4 space-y-4 shadow-md">
          {nav_links.map((item, index) => (
            <Link
              key={index}
              to={item.link}
              className={`block text-sm transition ml-8 ${isActive(item.link)
                ? "text-one font-bold"
